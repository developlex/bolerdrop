...
| Guest checkout placement | Partial | Guest checkout flow exists on dedicated `/checkout` page (shipping/payment/order placement), with fallback UX for zero shipping methods still pending. | `frontend/storefront/app/checkout/page.tsx`, `frontend/storefront/app/actions/checkout.ts` |
